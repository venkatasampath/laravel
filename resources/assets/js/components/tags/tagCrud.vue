    ...
    <v-dialog v-model="dialog" max-width="50%">
        <v-card>
            ...
            <v-card-text>
                <v-row>
                    <v-col cols="6">
                        <v-autocomplete dusk='tag-select-project' v-model="item.project_id" label="Select Project" :disabled="item.org || disabled || !isOrgAdmin"
                                        :items="this.projectsData" item-value="projectsValue" item-text="projectsText">
                        </v-autocomplete>
                    </v-col>
                    <v-col cols="6">
                        <v-switch dusk='tag-set-org' v-if="isOrgAdmin" v-model="item.org" primary label="Org Tag" :disabled="disabled"/>
                    </v-col>
                </v-row>
                <v-row>
                    <v-col cols="6"><v-text-field dusk='tag-name' v-model="item.name" label="Tag name" :disabled="disabled"/></v-col>
                    <v-col cols="6"><v-text-field dusk='tag-description' v-model="item.description" label="Description" :disabled="disabled"/>
                    </v-col>
                </v-row>
                <v-row>
                    <v-col cols="6"><v-text-field dusk='tag-category' v-model="item.category" label="Category" :disabled="disabled"/></v-col>
                    <v-col cols="6">
                        <v-autocomplete dusk='tag-set-type' v-model="item.type" label="Select Tag" :items="tag_type_items" :disabled="disabled || !isOrgAdmin"/>
                    </v-col>
                </v-row>
                <v-row>
                    <v-col cols="6">
                        <v-text-field v-model="tag_color" label="Color" :background-color="tag_color" :disabled="disabled">
                            <template v-slot:append-outer>
                                <v-btn dusk='tag-color-selector' class="homunculus-pen-color" small icon @click.stop="colorPickerDialog = true">
                                    <v-icon :color="tag_color">mdi-format-color-highlight</v-icon>
                                </v-btn>
                                <v-dialog dusk='tag-color-picker' v-model="colorPickerDialog" max-width="400px">
                                    <v-card>
...
<!--                    <v-col cols="6"><v-text-field v-model="item.icon" label="Icon" :disabled="disabled"/></v-col>-->
                    <v-col cols="6">
                        <v-text-field dusk="tag-icon" v-model="item.icon" label="Icon" :disabled="disabled" clearable>
                            <template v-slot:append-outer>
            ...
            <v-card-actions v-if="action.edit || action.create">
                <v-row align="center" justify="center" class="m-0 p-0">
                    <v-btn dusk="tag-save" color="primary" text @click="save"><v-icon class="p-2">mdi-content-save</v-icon>Save</v-btn>
                    <v-btn dusk="tag-cancel" color="primary" text @click="close"><v-icon class="p-2">mdi-cancel</v-icon>Cancel</v-btn>
                </v-row>
            </v-card-actions>
