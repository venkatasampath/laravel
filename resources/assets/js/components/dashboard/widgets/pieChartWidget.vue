    ...
    <v-card :class="setClass" v-if="show_widget">
        <v-card-title class="m-0 p-0">
            <v-toolbar dense>
                ...
                <v-btn :dusk="`${ title.toLowerCase().match(/^[^:]+:\s*/)}-size`" :title="show_pie_full?'Semi Circle':'Full Circle'" icon color="primary" @click="toggleCircleSize"><v-icon>{{ show_pie_full?'mdi-circle-half-full':'mdi-circle' }}</v-icon></v-btn>
                <v-btn :dusk="`${ title.toLowerCase().match(/^[^:]+:\s*/)}-donut`" :title="show_pie?'Show Donut':'Show Pie'" icon color="primary" @click="togglePieDoughnut"><v-icon>{{ show_pie?'mdi-chart-donut':'mdi-chart-pie' }}</v-icon></v-btn>
                <v-btn :dusk="`${ title.toLowerCase().match(/^[^:]+:\s*/)}-collapse`" :title="!isCollapse ? 'Collapse' : 'Expand'" icon color="primary" @click="collapse">
                    <v-icon v-text="isCollapse ? 'mdi-arrow-collapse-down' : 'mdi-arrow-collapse-up'"></v-icon>
                </v-btn>
                <v-btn :dusk="`${ title.toLowerCase().match(/^[^:]+:\s*/)}-close`" title="Close" icon color="primary" @click="close" :loading="loading"><v-icon>mdi-close</v-icon></v-btn>
            </v-toolbar>
        </v-card-title>
        ...
        <v-card-actions v-if="!isCollapse" class="m-0 p-0">
            <v-toolbar dense>
                ...
                <v-spacer></v-spacer>
                <v-btn :dusk="`${ title.toLowerCase().match(/^[^:]+:\s*/)}-details`" title="Details" icon color="primary" @click="fireGenerateDetails" v-if="showOptions"><v-icon>mdi-clipboard-text</v-icon></v-btn>
