    ...
    <v-card :class="setClass" v-if="show_widget">
        <v-card-title class="m-0 p-0">
            <v-toolbar dense>
                ...
                <v-btn  :dusk="`${ titles[type].toLowerCase() }-collapse`" :title="!isCollapse ? 'Collapse' : 'Expand'" icon color="primary" @click="collapse">
                    <v-icon v-text="isCollapse ? 'mdi-arrow-collapse-down' : 'mdi-arrow-collapse-up'"></v-icon>
                </v-btn>
                <v-btn :dusk="`${ titles[type].toLowerCase() }-close`" title="Close" icon color="primary" @click="close" :loading="loading"><v-icon>mdi-close</v-icon></v-btn>
            </v-toolbar>
        </v-card-title>
        ...
        <v-card-actions v-if="!isCollapse" class="m-0 p-0">
            <v-toolbar dense>
                ...
                <v-spacer></v-spacer>
                <v-btn :dusk="`${ titles[type].toLowerCase() }-details`" title="Details" icon color="primary" @click="fireGenerateDetails" v-if="showOptions"><v-icon>mdi-clipboard-text</v-icon></v-btn>
